...
export default function WagamamaPage() {
  const GAP_PX = 10;
  const TOPLINE_TARGET = 0.991;
  const CURRENT_ROUTE = "/wagamama";
  const ACTIVE_LABEL = "WAGAMAMA";
  ...
  const firstRow = useMemo(
    () => ({
      ...
      description:
        "Sandbox copy of the Christie’s case study for squiggle work (same as Givenchy sandbox). The live Christie’s page is unchanged.",
            ...
            <div className="flex flex-col" style={{ width: 581 }}>
              <div style={{ marginTop: 187 }}>
                ...
                <p className="text-[12px] leading-[1.5]">
                  <Link
                    href="http://localhost:3001/projects/wagamama-page.html"
                    className="underline"
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    Open timeline sandbox (scroll squiggle)
                  </Link>
                  {" · "}
